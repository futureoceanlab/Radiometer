...
--------------------------------------------------------------------------------
 ...
 Version:       Beebe
 Design:        RAD_Teensy
 Substrate:     Teensy 4.1
--------------------------------------------------------------------------------
 Module:        RAD_Teensy
 Filename:      RAD_Teensy.iso
 Created:       26/10/2019
 Author:        Allan Adams <allan@oceanic-labs.org>
--------------------------------------------------------------------------------
 Description:   RAD_Teensy is a Teensy 4.1 executable that operates the FOL/WHOI 
...
void errorHalt(const char* msg) {
  ...
  if (SD.sdfs.sdErrorCode()) {
    if (SD.sdfs.sdErrorCode() == SD_CARD_ERROR_ACMD41) {
      if (SERIAL_USB) {SERIAL_USB.println("Try power cycling the SD card.");}
    }
    printSdErrorSymbol(&SERIAL_USB, SD.sdfs.sdErrorCode());
    SERIAL_USB.print(", ErrorData: 0X");
    if (SERIAL_USB) {SERIAL_USB.println(SD.sdfs.sdErrorData(), HEX);}
  }
  //BuzzerDoom();  
  while (true) {} 
}
...
// INPUTS
  for (int i=0; i<8; ++i) { 
    pinMode(pin_DataLow[i],INPUT);
    pinMode(pin_DataHigh[i],INPUT);   
   }
...
// INTERRUPTS
  ...
  NVIC_SET_PRIORITY(IRQ_GPIO6789, 16); // All pin interrupts are on IRQ_GPIO6789, see https://forum.pjrc.com/threads/59828-Teensy-4-Set-interrupt-priority-on-given-pins
...
//  digitalWrite(pin_CAN0_Lo  ,HIGH); // Low power mode == Pin --> VCC
  ...
  digitalWrite(pin_EN_CMOD, HIGH);
  while (!digitalRead(pin_CMODRdy)) {}
...
int  Open_Files() { // DONE  
  ...
  File root = SD.open("/");
  SD.sdfs.chdir("/");
  SD.sdfs.mkdir(Filename_Time);
  SD.sdfs.chdir(Filename_Time);
...
//          Set Dtog Low and read 16-bit bus into local buffer
    SD_Data_Buffer16[2] = ReadDataBus();
...
//    SD_Data_Buffer8[7] = (uint8_t) (GPIOD_PDIR & 0xFF); // ~20ns
    SD_Data_Buffer16[3] = ReadDataBus();
...
uint16_t ReadDataBus() {
  static uint16_t out;
  out = digitalReadFast(DATA00) +
        (digitalReadFast(DATA01) << 1)  +
        (digitalReadFast(DATA02) << 2 ) +
        (digitalReadFast(DATA03) << 3 ) +
        (digitalReadFast(DATA04) << 4 ) +
        (digitalReadFast(DATA05) << 5 ) +
        (digitalReadFast(DATA06) << 6 ) +
        (digitalReadFast(DATA07) << 7 ) +
        (digitalReadFast(DATA08) << 8 ) +
        (digitalReadFast(DATA09) << 9 ) +
        (digitalReadFast(DATA10) << 10) +
        (digitalReadFast(DATA11) << 11) +
        (digitalReadFast(DATA12) << 12) +
        (digitalReadFast(DATA13) << 13) +
        (digitalReadFast(DATA14) << 14) +
        (digitalReadFast(DATA15) << 15);
  return out;
}
